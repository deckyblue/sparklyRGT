...
parameters {
        ...
        vector[N] beta_samp;
        vector[N] eta_samp;
        vector[N] m_samp; // slope
        vector[N] b_samp; // intercept
        ...
        for (d in 1:1) {
                ...
                for (tr in 1:ntr) {
                        ...
                        if ( O[tr]>0 ) {
                                ...
                                if ( P[tr]>0 ) {
                                        delta = -m[currentSubject]*P[tr] + b[currentSubject] - Q[O[tr]];
                                }
...
model = stan.build(pscale_star_model, data = model_data, random_seed=1)

#do the sampling
fit = model.sample(num_chains=4)
fit_az = az.from_pystan(fit)
##change argument (task_model_fit.nc)
fit_az.to_netcdf('standard_pscale-star_fit.nc')